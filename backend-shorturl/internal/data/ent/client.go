...
import (
	...
	"backend-shorturl/internal/data/ent/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.Shorturl.Intercept(interceptors...)
}

// Mutate implements the ent.Mutator interface.
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *ShorturlMutation:
		return c.Shorturl.mutate(ctx, m)
	default:
...
// A call to `Intercept(f, g, h)` equals to `shorturl.Intercept(f(g(h())))`.
func (c *ShorturlClient) Intercept(interceptors ...Interceptor) {
	c.inters.Shorturl = append(c.inters.Shorturl, interceptors...)
}

// Create returns a builder for creating a Shorturl entity.
...
func (c *ShorturlClient) MapCreateBulk(slice any, setFunc func(*ShorturlCreate, int)) *ShorturlCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ShorturlCreateBulk{err: fmt.Errorf("calling to ShorturlClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ShorturlCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *ShorturlClient) UpdateOne(s *Shorturl) *ShorturlUpdateOne {
	mutation := newShorturlMutation(c.config, OpUpdateOne, withShorturl(s))
	return &ShorturlUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ShorturlClient) UpdateOneID(id int64) *ShorturlUpdateOne {
...
func (c *ShorturlClient) DeleteOne(s *Shorturl) *ShorturlDeleteOne {
	return c.DeleteOneID(s.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ShorturlClient) DeleteOneID(id int64) *ShorturlDeleteOne {
...
func (c *ShorturlClient) Query() *ShorturlQuery {
	return &ShorturlQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeShorturl},
		inters: c.Interceptors(),
	}
}

// Get returns a Shorturl entity by its id.
func (c *ShorturlClient) Get(ctx context.Context, id int64) (*Shorturl, error) {
	return c.Query().Where(shorturl.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ShorturlClient) GetX(ctx context.Context, id int64) *Shorturl {
...
func (c *ShorturlClient) Interceptors() []Interceptor {
	return c.inters.Shorturl
}

func (c *ShorturlClient) mutate(ctx context.Context, m *ShorturlMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ShorturlCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ShorturlUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ShorturlUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ShorturlDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Shorturl mutation op: %q", m.Op())
	}
}
